...
module.exports = {
  ...
  checkout: async (req, res) => {
    try {
      ...
      if (!res_nominal) return res.status(404).json({mesage: 'Nominal tidak ditemukan'});

      const res_payment = await Payment.findOne({_id: payment});
      if (!res_payment) return res.status(404).json({mesage: 'Jenis pembayaran tidak ditemukan'});

      const res_bank = await Bank.findOne({_id: bank});
      if (!res_bank) return res.status(404).json({mesage: 'Bank tidak ditemukan'});
      ...
      const payload = {
        ...
        category: res_voucher._id.category?._id,
  ...
  history: async (req, res) => {
    try {
      ...
      let totalTransaksi = await Transaction.aggregate([
        {$match: criteria},
        {
          $group: {
            _id: null,
      ...
      res.status(200).json({
        data: history,
        total: totalTransaksi.length ? totalTransaksi[0].value : 0,
      });

    } catch (err) {
      res.status(500).json({message: err.mesage || 'Internal server error'});
    }
  },
  ...
  dashboard: async (req, res) => {
    try {
      ...
      const history = await Transaction.find({palyer: req.player._id})
        .populate('category')
        .sort({'updatedAt': -1});
  ...
  editProfile: async (req, res, next) => {
    try {
      ...
      if (req.file) {
        ...
        const player = await Player.findOneAndUpdate({_id: req.player._id}, payload, {
          new: true,
          runValidators: true
        });
